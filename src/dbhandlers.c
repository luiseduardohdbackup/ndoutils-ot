...
int ndo2db_handle_servicecheckdata(ndo2db_idi *idi){
	...
	/* get the notifications_enabled status and host alias */
	// Select the correct alias and notification state from DB
	if(asprintf(&buf,"SELECT %s.alias, %s.notifications_enabled, SUBSTRING_INDEX(%s.display_name, ':', 1) FROM %s, %s, %s WHERE service_object_id='%lu' and %s.host_object_id = %s.host_object_id and %s.host_object_id = %s.host_object_id limit 1"
	...
	MYSQL_RES *res = mysql_store_result(&idi->dbinfo.mysql_conn);
 	MYSQL_ROW row;

	if(row=mysql_fetch_row(res)){
		es[5] = ndo2db_db_escape_string(idi,row[0]);
		ndo2db_convert_string_to_int(row[1],&notifications_enabled);
		es[6] = ndo2db_db_escape_string(idi,row[2]);
	}else{
		mysql_free_result(res);
		free(row);
		if(asprintf(&buf,"SELECT %s.alias, %s.notifications_enabled, SUBSTRING_INDEX(%s.display_name, ':', 1) FROM %s, %s, %s, %s WHERE object_id='%lu' and %s.host_object_id = %s.host_object_id and %s.host_object_id = %s.host_object_id and name1 = alias limit 1"
			    ,ndo2db_db_tablenames[NDO2DB_DBTABLE_HOSTS]
			    ,ndo2db_db_tablenames[NDO2DB_DBTABLE_HOSTSTATUS]
			    ,ndo2db_db_tablenames[NDO2DB_DBTABLE_SERVICES]
			    ,ndo2db_db_tablenames[NDO2DB_DBTABLE_HOSTS]
